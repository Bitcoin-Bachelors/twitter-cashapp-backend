...
from .task import twitter_get_oauth_request_token, twitter_get_user_authorization, twitter_get_access_token, cashapp_authenticate_application
...
class InitiateCashAppAuth(APIView):
    ...
    def get(self, request):
        request_token_response = cashapp_authenticate_application()
        response = HttpResponseRedirect(redirect_to=settings.NEXT_REDIRECT_URL)
        response.set_cookie('access_token', json.dumps(request_token_response))
        return response
